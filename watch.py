"""Reforger Watch v0.1 — Python 3.10+, standard library only."""
import argparse
import hashlib
import json
import logging
...
LOG = logging.getLogger('reforger-watch')
...
class Watch:
    ...
    def send(self, payload):
        ...
        request = Request(self.webhook + ('&' if '?' in self.webhook else '?') + 'wait=true', data=body, headers={'Content-Type': content_type, 'User-Agent': 'reforger-watch/0.1'}, method='POST')
        try:
...
def main():
    ...
    watcher = Watch(os.getenv('DATABASE_PATH','./data/reforger-watch.db'), server_id, os.getenv('SERVER_LABEL','WCS NA7'), API(os.getenv('REFORGERMODS_BASE_URL','https://api.reforgermods.net/v2'),os.getenv('CLIENT_NAME','reforger-watch/0.1')), webhook, os.getenv('DONATION_URL') or None)
    try:
        if args.status:
